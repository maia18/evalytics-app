...
# Cores das barras do gráfico de eixos. Segunda e terceira cores são decorativas
# (verde/vermelho de gráfico), não fazem parte da paleta de tema claro/escuro.
...
def ViewDashboard(page: ft.Page, mudar_tela: Callable[[str], None]) -> ft.View:
    ...
    # TODO: assim como a tabela e o gráfico de desempenho, estes dados dos KPIs e do
    # gráfico de eixos podem ser substituídos por funções do Firestore futuramente.
    dados_kpi = {
        "avaliacoes_ativas": "4",
    ...
    linha_kpis = ft.Row(
        wrap=True,  # Responsividade: cards descem para a próxima linha se a tela encolher
        spacing=20,
        run_spacing=20,
        controls=[
    ...
    # IMPORTANTE ARQUITETURAL: expand=True no Container (limite de altura vindo do
    # TabBarView), scroll=AUTO isolado na Column interna, sem expand=True nela mesma.
    # Evita o bug do Flet de espaço em branco + scroll fantasma quando expand e scroll
    # são combinados na mesma Column (github.com/flet-dev/flet/issues/6087).
    ...
    conteudo_abas = ft.TabBarView(
        ...
        controls=[
            ...
            criar_conteudo_avaliacoes(layout, mudar_tela, page),  # Reaproveita a tela inteira de Avaliações
        ],
    )
