...
def _criar_campo_texto(
    label: str, hint_text: str, cor_titulo: str, cor_secundario: str, cor_borda: str, **kwargs,
) -> ft.Column:
    """Casca compartilhada dos campos de texto (Factory).
    
    `**kwargs` permite que particularidades de cada campo (como password=True) sejam injetadas sem repetir código.
    """
    return ft.Column(
        spacing=5,
        controls=[
            ...
            ft.TextField(
                hint_text=hint_text, hint_style=ft.TextStyle(color=cor_secundario),
                border_color=cor_borda, border_radius=8, content_padding=15,
                cursor_color=cor_titulo, text_style=ft.TextStyle(color=cor_titulo), **kwargs,
            ),
        ],
    )
...
def criar_campo_nome(cor_texto_titulo: str, cor_texto_secundario: str, cor_borda: str) -> ft.Column:
    ...
    campo.visible = False  # A tela abre por padrão em modo "Sign In" logo não solicita nome.
    return campo
...
def criar_campo_email(cor_texto_titulo: str, cor_texto_secundario: str, cor_borda: str) -> ft.Column:
    ...
    return _criar_campo_texto("Email", "voce@instituicao.com", cor_texto_titulo, cor_texto_secundario, cor_borda)
