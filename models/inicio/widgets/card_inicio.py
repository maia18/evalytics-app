...
def criar_card(
    ...
    Args:
        layout: instância do layout atual (usada para extrair as cores do tema).
        titulo: nome em destaque do atalho (ex: "Dashboard").
        descricao: texto explicativo menor abaixo do título.
        icone: constante do ícone (Flet/Material Design).
        rota: caminho interno para onde o clique deve levar (ex: "/dashboard").
        mudar_tela: callback responsável por executar a troca de rota.
    """
    seta_indicadora = ft.Icon(
        ft.Icons.ARROW_FORWARD_ROUNDED,
        color=ft.Colors.TRANSPARENT,  # Invisível por padrão; aparece no hover
        size=18,
        animate_opacity=200,
    )
