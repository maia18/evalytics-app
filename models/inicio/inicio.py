...
def ViewInicio(page: ft.Page, mudar_tela: Callable[[str], None]) -> ft.View:
    ...
    # Inicializa o ResponsiveLayout consumindo as constantes de texto de TXTS_INICIO.
    layout = ResponsiveLayout(
        page, TXTS_INICIO[0], TXTS_INICIO[1], mudar_tela=mudar_tela,
    )

    # Criação do "Hero Banner" (Bloco de destaque principal superior).
    hero = ft.Container(
    ...
    # O ResponsiveRow ajusta a quantidade de colunas automaticamente dependendo do tamanho da tela (xs, sm, md, lg).
    ...
    conteudo = ft.Column(expand=True, controls=[hero, ft.Container(height=24), grade_cards])
