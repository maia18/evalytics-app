...
class FormularioRenderMixin:
    """Mixin responsável por ler o estado atual do Controller e refleti-lo na tela."""
    ...
    def atualizar_renderizacao(self) -> None:
        ...
        btn_cancelar = ft.TextButton(
            "Cancelar", icon=ft.Icons.CANCEL, icon_color="error", style=ft.ButtonStyle(color="error"),
            on_click=lambda _: self.mudar_tela("/inicio"),
        )
        btn_anterior = ft.ElevatedButton(
            "Anterior", icon=ft.Icons.ARROW_BACK, bgcolor="surfaceVariant", color="onSurface",
            disabled=(self.estado["indice_atual"] == 0), on_click=self.anterior,
        )
        ...
        rodape = ft.Container(
            ...
            content=ft.Row(
                alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                controls=[btn_cancelar, ft.Row([btn_anterior, btn_avancar], spacing=10)],
            ),
        )
